...
一款自动将研究论文上传至 [千问阅读](https://www.qianwen.com/read) 并通过 GPT-4o-mini 根据用户自定义问题进行自动判断的工具。

## 功能

- **自动上传**: 自动将 `./paper` 中的 PDF 论文上传到千问阅读。
- **自动摘要提取**: 自动在网页界面抓取其“AI 导读”的摘要。
- **自定义 AI 分析**: 使用 GPT-4o-mini 对摘要进行分析，并根据用户传入的问题给出答案。
- **结果导出**: 将 `title`, `summary` 和 `answer` 结果导出为 CSV 文件。
...
2. **Tokens**: 在 `auto_read` 目录下（或根目录下）创建 `tokens.yaml` 文件，填入你的 OpenAI API Key 和 base_url。
...
├── auto_read/
│   ├── auto_read.py     # 主运行程序
│   ├── cookies.json     # 用户 cookies
│   ├── tokens.yaml      # API tokens
│   └── chat_screenshots/# 存放生成的核对截图
└── answer.csv           # 最终导出的表格 (包含 title, summary, answer)
```
